...
public class HDFSApi8 {
	 ...
	 * 追加文本内容
	 */
	public static void appendContentToFile(Configuration conf, String content, String remoteFilePath)
			throws IOException {
		FileSystem fs = FileSystem.get(conf);
	 ...
	 * 移动文件到本地 移动后，删除源文件
	 */
	public static void moveToLocalFile(Configuration conf, String remoteFilePath, String localFilePath)
			throws IOException {
		FileSystem fs = FileSystem.get(conf);
		Path remotePath = new Path(remoteFilePath);
		...
		try {
			...
			if (!HDFSApi8.test(conf, remoteFilePath)) {
				...
				if (choice.equals("after")) { // 追加在文件末尾
					...
					String localTmpPath = "/home/hadoop/tmp.txt";
					// 移动到本地
